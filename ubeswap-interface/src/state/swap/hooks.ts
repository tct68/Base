...
import { CELO, cEUR, ChainId as UbeswapChainId, USDT, JSBI, Token, TokenAmount, Trade } from '@ubeswap/sdk'
...
export function useDerivedSwapInfo(): {
  ...
  if (balanceIn && amountIn && balanceIn.lessThan(amountIn)) {
    if (
      amountIn.currency.address === USDT[network.chainId as unknown as UbeswapChainId].address ||
...
function parseCurrencyFromURLParameter(urlParam: any, chainId: UbeswapChainId): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'CUSD') return USDT[chainId].address
    if (valid === false) return USDT[chainId].address
  }  
  return USDT[chainId].address ?? ''
}
