...
import { ChainId, USDT, JSBI, Token, TokenAmount } from '@ubeswap/sdk'
...
export const useAnnualRewardDollars = (rewardTokens: Token[], rewardRates: TokenAmount[]) => {
  ...
  return rewardPrices.reduce((acc, rewardPrice, idx) => {
    ...
    return acc.add(rewardPrice?.quote(rewardTokenPerYear) ?? new TokenAmount(USDT[chainId], '0'))
  }, new TokenAmount(USDT[chainId], '0'))
}
