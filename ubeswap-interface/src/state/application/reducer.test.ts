...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BaseMainnet, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.BaseMainnet]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BaseMainnet, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.BaseMainnet]).toEqual(3)
    })
    it('works with non-set chains', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BaseGoerli, blockNumber: 2 }))
      expect(store.getState().blockNumber).toEqual({
        [ChainId.BaseMainnet]: 3,
        [ChainId.BaseGoerli]: 2,
      })
    })
  })
