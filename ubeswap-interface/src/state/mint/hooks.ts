...
import { CELO, cEUR, ChainId, USDT, JSBI, Pair, Percent, Price, Token, TokenAmount } from '@ubeswap/sdk'
...
export function useDerivedMintInfo(
  ...
  if (currencyAAmount && currencyBalances?.[Field.CURRENCY_A]?.lessThan(currencyAAmount)) {
    if (
      currencyAAmount.currency.address === USDT[network.chainId as unknown as ChainId].address ||
  ...
  if (currencyBAmount && currencyBalances?.[Field.CURRENCY_B]?.lessThan(currencyBAmount)) {
    if (
      currencyBAmount.currency.address === USDT[network.chainId as unknown as ChainId].address ||
